 ...
 *
 * â€” async functions support
 * â€” fixed issues derived from lgtm static analysis
 * â€” unsortable items are no more deleted from bound array
 * â€” $.my.f.tojson generates prettier output, also
...
;(function ($) {
    ...
    var MY = {
        ...
        vals: {
            ...
            '.my-form-list': (function ($o,list) {
                ...
                // sortable hides unmovable items, so we canâ€™t use option/items. 1.3.3
    ...
    function _tojson(obj, x, y) {
        ...
        function _put(){
            var akey = key !== null && (
                !jsok || !/^[a-zÐ°-ÑÎ±-Ï‰$_][a-zÐ°-ÑÎ±-Ï‰$_0-9]*$/i.test(key)
    ...
    function _unjsonNode (i, node, incl) {
        ...
        if (/^(ob|ar)/.test(t)) _unjson(nd);
            ...
            a = nd.match(/^function([\s\n\t]+[\wÐ°-ÑÐ-Ð¯Î±-Ï‰Î‘-Î©_$][\wÐ°-ÑÐ-Ð¯Î±-Ï‰Î‘-Î©\d_$]*)?[\s\n\t]*\(([^\)]*)\)\s*\{([\s\S]*)\}[\s\n\r\t]*$/);
 ...
 *         nose:        "", left|right|top|bottom â€“ where to put nose (also known as arrow or beak)
