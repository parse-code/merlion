...
class Hooks
{
        ...
        do {
            foreach (current($this->filters[$tag]) as $theCurrent) {
                if (!is_null($theCurrent['function'])) {
                    $args[1] = $value;
                    $value   = call_user_func_array($theCurrent['function'],
                        array_slice($args, 1, (int)$theCurrent['accepted_args']));
                }
            }
    ...
    public function apply_filters_ref_array($tag, $args)
    {
        ...
        do {
            foreach ((array)current($this->filters[$tag]) as $theCurrent) {
                if (!is_null($theCurrent['function'])) {
                    $args[0] = call_user_func_array($theCurrent['function'],
                        array_slice($args, 0, (int)$theCurrent['accepted_args']));
                }
            }
        } while (next($this->filters[$tag]) !== false);
    ...
    public function do_action_ref_array($tag, $args)
    {
        ...
        do {
            foreach ((array)current($this->filters[$tag]) as $theCurrent) {
                if (!is_null($theCurrent['function'])) {
                    call_user_func_array($theCurrent['function'],
                        array_slice($args, 0, (int)$theCurrent['accepted_args']));
                }
            }
        } while (next($this->filters[$tag]) !== false);
